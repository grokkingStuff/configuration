...
Since pyenv is a bunch of shell scripts, we'll include it with the install src blocks.
...
#+END_SRC
 ...
 pyenv uses ~/.pyenv as its root directory by default. 
 This is the directory where pyenv installs its shims, the different Python versions and its plugins. 
 The default location is absolutely fine, but if you want to use the directory suggested by Homebrew then add the following to your profile.

#+NAME: bashrc
#+BEGIN_SRC sh :padline no :tangle no
